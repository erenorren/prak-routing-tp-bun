...
fetch(request) {
    ...
    // Routing manual
    if (path === '/' && method === 'GET') {
        return new Response('<h1>🏠 Halaman Utama (Bun)</h1><p>Selamat datang
    di server Bun + TypeScript!</p>', {
    headers: { 'Content-Type': 'text/html' },
    });
    }
else if (path === '/about' && method === 'GET') {
return new Response('<h1>📄 Tentang Kami (Bun)</h1><p>Routing manual
dengan Bun sangat mudah!</p>', {
headers: { 'Content-Type': 'text/html' },
});
}
